...
// This script was modified to suit modern PHP scripts, where the functions are encapsulated in a class structure as methods 
// and the static variables are initialized to null. Now this requires PHP 5 or newer due to the class methods "public" declaration 
...
Special thanks to alanjstr for cleaning up the code, especially on public static function self::get_item_version(),
...
This change will will NOT break your existing programming or browser detection public static function calls.
******************************************
...
argument used in the public static function call if used, like: browser_detection( 'full', '1' );
...
// main script, uses two other functions, get_os_data() and self::get_item_version() as needed
...
class BrowserDetection{
	...
	const UA_TYPE_BOT = 'bot';
	const UA_TYPE_BROWSER = 'bro';
	const UA_TYPE_LIMITED_BROWSER = 'bbro';
	const UA_TYPE_MOBILE_BROWSER = 'mobile';
	const UA_TYPE_DOWNLOAD_AGENT = 'dow';
	const UA_TYPE_CODE_LIBRARY = 'lib';
	
	public function __construct($debug){
		$this->_debug = $debug;
		$result_array = $this->browser_detection('full');
		
		$clean_array['browser_engine'] = $result_array[0];
		$clean_array['browser_version'] = $result_array[1];
		$clean_array['browser_major_number'] = $result_array[9];
		$clean_array['browser_name'] = $result_array[7];
		$clean_array['os_type'] = $result_array[5];
		$clean_array['os_version'] = $result_array[6];
		$clean_array['ua_type'] = $result_array[8];
		$clean_array['ie_version'] = $result_array[2];
		$clean_array['mozilla_data'] = $result_array[10];
		$clean_array['webkit_data'] = $result_array[11];
		$clean_array['mobile_data'] = $result_array[13];
		
		$this->_results = new Model($clean_array);
	}
	
	public function getResults(){ $this->_results; }
	
	public function isStandardBrowser(){ return ($this->_results->get('ua_type') == self::UA_TYPE_BROWSER); }	
	public function isBot(){ return ($this->_results->get('ua_type') == self::UA_TYPE_BOT); }
	public function isMobile(){ return ($this->_results->get('ua_type') == self::UA_TYPE_MOBILE_BROWSER); }	
	...
	public static function browser_detection( $which_test, $test_excludes='', $external_ua_string='' ) 
	{
		...
		*/
		...
		static 
			...
			$browser_name = null, 
			$browser_number = null, 
			$browser_math_number = null, 
			$browser_user_agent = null, 
			$browser_working = null, 
			$html_type = null, 
			$ie_version = null, 
			$khtml_type = null, 
			$khtml_type_number = null, 
			$mobile_test = null, 
			$moz_type_number = null, 
			$moz_rv = null, 
			$moz_rv_full = null, 
			$moz_release_date = null, 
			$moz_type = null, 
			$os_number = null, 
			$os_type = null, 
			$layout_engine = null, 
			$layout_engine_nu = null, 
			$layout_engine_nu_full = null, 
			$true_ie_number = null, 
			$ua_type = null, 
			$webkit_type = null, 
			$webkit_type_number = null;
		...
		*/
		if ( !$b_repeat ) {
			...
			*/
			...
			to true so the public static function won't retest everything
			*/
			$b_repeat = true;
			/*
	...
	public static function get_item_math_number( $pv_browser_number )
	{
		$browser_math_number = '';
	...
	/* 
	Function Info:
	public static function returns browser number, gecko rv number, or gecko release date
	public static function self::get_item_version( $browser_user_agent, $search_string, $substring_length )
	...
	*/
	public static function check_is_mobile( $pv_browser_user_agent )
	{
		...
		This list is almost the same but not exactly as the 4 arrays in public static function below
		*/
	...
	*/
	...
	public static function get_html_level( $pv_render_engine, $pv_render_engine_nu )
	{	
		$html_return = 1;
