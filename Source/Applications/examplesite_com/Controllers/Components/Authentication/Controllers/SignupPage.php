<?php
exit; 
class SignupPage extends TemplatedPageRequest implements IJSONRequest{
...
//			$this->redirect('/Account/Welcome',200,true); 
		...
		$RoleCollection = RoleActions::selectAll();
		foreach($RoleCollection as $Role){
			if($Role->getData('is_user_selectable','Is::set')){
				$this->_data['roles'][$Role->get('role_id')] = $Role->get('title');
			}
		}
		...
		if(isset($_POST['registration']))
		{
			...
			$role_ids = $this->Input->get('role_ids');
			if(!is_array($role_ids) || array_contains($role_ids, $this->_data['roles'])){
				$this->Errors->set('role_ids', 'You must select a role, parent or child, before continuing.');
			}
					
// 			stop annoying people with this. they can reset their passwords if they lose them
//			if($this->Input->getData('password','nothing') !== $this->Input->getData('confirm_password','nothing'))
//			{ $this->Errors->set('confirm_password','The password you entered did not match your confirmed password.'); }
...
//			}
			...
			if($this->Errors->length()<1)
			{
				...
				$user_id = RegistrationActions::insertUser($this->Input);
...
//					$this->redirect('/Account/Login/',200,true);
	...
	public function renderPage(){
		...
		$this->addCss('pages/Account/Signup');
		$this->addJs('pages/Account/Signup');
		
		/* page logic */
		
		// or if you're running a page that uses PHP instead of HTML templates, run it this way
		$this->_page_body = $this->runCodeReturnOutput('pages/Account/Signup/layout.php');
	}
}
