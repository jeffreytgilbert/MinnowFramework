...
class ChildSignUpPage extends TemplatedPageRequest implements IJSONRequest{
...
//			$this->redirect('/Account/Welcome',200,true); 
		...
		if(isset($_POST['registration']))
		{
			...
			if(!($this->Input->getData('age','') >= 6 && $this->Input->getData('age','') <= 14)){
				$this->Errors->set('age', 'You must be between 6 and 14 years old.');
			}
...
//			}
			...
			if($this->Errors->length()<1)
			{
				...
				$user_id = RegistrationActions::insertChild($this->Input);
...
//					$this->redirect('/Account/Login/',200,true);
	...
	public function renderPage(){
		...
		$this->addCss('pages/Account/ChildSignUp');
		$this->addJs('pages/Account/ChildSignUp');
		
		/* page logic */
		
		// or if you're running a page that uses PHP instead of HTML templates, run it this way
		$this->_page_body = $this->runCodeReturnOutput('pages/Account/ChildSignUp/layout.php');
	}
}
