...
class UserAvatar extends DataObject{
	...
	public static function cast(DataObject $DataObject){
		return ($DataObject instanceof UserAvatar)?$DataObject:new UserAvatar($DataObject->toArray());
	}
	...
	public function getAvatarPart(){
		return ($this->getObject('AvatarPart') instanceof AvatarPart)
			?$this->_data['AvatarPart']
			:new AvatarPart();
	}
	
}

class UserAvatarCollection extends DataCollection{
	public function __construct(Array $array_of_objects=null){
		$this->setCollectionType('UserAvatar');
	...
	public function getUserAvatarByFieldValue($column_name, $data_value, $return_blank_object_if_null=true){
		$return = parent::getObjectByFieldValue($column_name, $data_value, $return_blank_object_if_null);
		return ($return instanceof UserAvatar)?$return:new UserAvatar($return->toArray());
	}
	
	public function getUserAvatarByIndex($index, $return_blank_object_if_null=true){
