<?php

/*
 * This work is licensed under the Creative Commons Attribution-ShareAlike 3.0 Unported License. To view a copy of this license, visit http://creativecommons.org/licenses/by-sa/3.0/.
 */

class Setting extends DataObject{
	public function __construct(Array $data=array()){
		$this->addAllowedData(array(
			'setting_id'=>DataType::NUMBER,
			'setting_group_id'=>DataType::NUMBER,
			'SettingGroup'=>DataType::OBJECT,
			'type'=>DataType::TEXT,
			'name'=>DataType::TEXT,
			'hint'=>DataType::TEXT,
			'default_value'=>DataType::TEXT,
			'is_active'=>DataType::BOOLEAN,
			'is_on_by_default'=>DataType::BOOLEAN,
			'is_shown_at_signup'=>DataType::BOOLEAN,
			'created_datetime'=>DataType::DATETIME
		),true);
		parent::__construct($data);
	}

	public static function cast(DataObject $DataObject){
		return ($DataObject instanceof Setting)?$DataObject:new Setting($DataObject->toArray());
	}
	
	// Static typed child methods for autocomplete on object
	
	
	public function getSettingGroup(){
		return ($this->getObject('SettingGroup') instanceof SettingGroup)
			?$this->_data['SettingGroup']
			:new SettingGroup();
	}
	
}

class SettingCollection extends DataCollection{
	public function __construct(Array $array_of_objects=null){
		$this->setCollectionType('Setting');
		parent::__construct($array_of_objects);
	}
	
	// Add autocomplete helper methods to the only two ModelCollection methods that return an object
	
	public function getSettingByFieldValue($column_name, $data_value, $return_blank_object_if_null=true){
		$return = parent::getObjectByFieldValue($column_name, $data_value, $return_blank_object_if_null);
		return ($return instanceof Setting)?$return:new Setting($return->toArray());
	}
	
	public function getSettingByIndex($index, $return_blank_object_if_null=true){
		$return = parent::getObjectByIndex($index, $return_blank_object_if_null);
		return ($return instanceof Setting)?$return:new Setting($return->toArray());
	}
	
	// Static typed child methods for autocomplete on collection
}